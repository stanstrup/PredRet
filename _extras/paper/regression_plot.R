...
# correlation plot
reg_model <- lm(data_sub$predicted_rt ~ data_sub$recorded_rt)
rreg_model <- rlm(data_sub$predicted_rt ~ data_sub$recorded_rt)
...
p <- p + annotate("text",x=0,y=20,hjust=-0.2,vjust=0,label= paste0("R^{2}==",round(summary(reg_model)$r.squared,3) ),parse = TRUE         )
...
# error
plot( data_sub$recorded_rt,       (data_sub$predicted_rt-data_sub$recorded_rt)   ,pch=20,ylim=c(-5,5))


# rel error
plot( data_sub$recorded_rt,       (data_sub$predicted_rt-data_sub$recorded_rt)/data_sub$recorded_rt    ,ylim=c(-1,1) ,pch=20)
