...
makeRedirect <- function(appnames, baseurl) {
  CPU <- read.table("/srv/shiny-server/shiny.apps/apps/retpred_shiny/retdb_redirector/Data/CPU.txt") # 
...
pwd <- function() {
  ...
  path = str_replace(path,"_redirector","")
  return(path)
}
...
shinyServer(function(input, output, session) {
  ...
  list(
       ...
       textInput(inputId = "url", label = "", value = makeRedirect(
                                                      ...
                                                      "http://predret.org/shiny/shiny.apps/apps/retpred_shiny/retdb_symlinks/")),
