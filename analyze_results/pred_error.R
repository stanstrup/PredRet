## CI width
hist((test[,"ci_upper"]-test[,"ci_lower"]),20,ylim=c(0,70))
hist((test[is.na(test[,"recorded_rt"]),"ci_upper"]-test[is.na(test[,"recorded_rt"]),"ci_lower"]),20,ylim=c(0,70))
...
par(mar=c(5,14,4,2))
plot(c(min(test_know[,c("recorded_rt","ci_lower")]),max(test_know[,c("recorded_rt","ci_upper")])), c(1,nrow(test_know)), type='n', xlab='RT', ylab='', axes=FALSE)
axis(1)
